...
class SMOTE(object):
    ...
    def fit(self, X):
        ...
        neighbors = self.k + 1
        self.neigh = NearestNeighbors(n_neighbors= neighbors)
...
class SMOTEBoost(AdaBoostClassifier):
    ...
    def fit(self, X, y, sample_weight=None, minority_target=None):
        ...
        # Check parameters.
        self.smote = SMOTE(k_neighbors=self.k_neighbors,random_state=self.random_state)
...
class RUSBoost(AdaBoostClassifier):
    ...
    def fit(self, X, y, sample_weight=None, minority_target=None):
        ...
        # Check parameters.
        self._validate_estimator()
        self.rus = RandomUnderSampler(with_replacement=self.with_replacement,
                                      return_indices=True,
...
class SMOTEBagging(BaseEstimator):
    def __init__(self,
                 n_samples=100,
                 min_ratio=1.0,
                 with_replacement=True,
...
class UnderBagging(BaseEstimator):
    def __init__(self,
                 n_samples=100,
                 min_ratio=1.0,
                 with_replacement=True,
...
class BalanceCascade(BaseEstimator):
    ...
    def fit(self, X, y, print_log=False, visualize=False):
        ...
        self.model_list = []
        df = pd.DataFrame(X); df['label'] = y
        if len(df[y == 0]) > len(df[y == 1]):
            df_maj = df[y==0]; n_maj = df_maj.shape[0]
            df_min = df[y==1]; n_min = df_min.shape[0]
        else:
            df_maj = df[y==1]; n_maj = df_maj.shape[0]
            df_min = df[y==0]; n_min = df_min.shape[0]
